from graphics import Window, Line, Point

# cell class hold the data about indivisual cell
class Cell:
    def __init__(self, win=None):
        self.has_left_wall = True
        self.has_right_wall = True
        self.has_top_wall = True
        self.has_bottom_wall = True
        self.visited = False
        self._x1 = None
        self._x2 = None
        self._y1 = None
        self._y2 = None
        self._win = win

    # to draw the cell
    def draw(self, x1, y1, x2, y2):
        if self._win is None:
            return
        self._x1 = x1
        self._y1 = y1
        self._x2 = x2
        self._y2 = y2

        
        if(self.has_left_wall):
            l = Line(Point(x1, y1), Point(x1, y2))
            self._win.draw_Line(l)
        else:
            l = Line(Point(x1, y1), Point(x1, y2))
            self._win.draw_Line(l, "white")

        if(self.has_top_wall):
            l = Line(Point(x1, y1), Point(x2, y1))
            self._win.draw_Line(l)
        else:
            l = Line(Point(x1, y1), Point(x2, y1))
            self._win.draw_Line(l, "white")

        
        if(self.has_right_wall):
            l = Line(Point(x2, y2), Point(x2, y1))
            self._win.draw_Line(l)
        else:
            l = Line(Point(x2, y2), Point(x2, y1))
            self._win.draw_Line(l, "white")
        
        if(self.has_bottom_wall):
            l = Line(Point(x1, y2), Point(x2, y2))
            self._win.draw_Line(l)
        else:
            l = Line(Point(x1, y2), Point(x2, y2))
            self._win.draw_Line(l, "white")


    # draw a path between 2 cells. It should draw a line from the center of one cell to another
    def draw_move(self, to_cell, undo = False):
        if self._win is None:
            return
        
        fill_color = "Blue"
        if undo:
            fill_color = "gray"

        # find the mid point of the  first box
        x_mid = (self._x1 + self._x2)/2
        y_mid = (self._y1 + self._y2)/2

        # find the mid point of the  first box

        to_x_mid = (to_cell._x1 + to_cell._x2) / 2
        to_y_mid = (to_cell._y1 + to_cell._y2) / 2


        # move left
        if self._x1 > to_cell._x1:
            line = Line(Point(self._x1, y_mid), Point(x_mid, y_mid))
            self._win.draw_Line(line, fill_color)
            line = Line(Point(to_x_mid, to_y_mid), Point(to_cell._x2, to_y_mid))
            self._win.draw_Line(line, fill_color)

        # move right
        elif self._x1 < to_cell._x1:
            line = Line(Point(x_mid, y_mid), Point(self._x2, y_mid))
            self._win.draw_Line(line, fill_color)
            line = Line(Point(to_cell._x1, to_y_mid), Point(to_x_mid, to_y_mid))
            self._win.draw_Line(line, fill_color)
        
        # moving top
        elif(self._y1 > to_cell._y1):
            line = Line(Point(x_mid, y_mid), Point(x_mid, self._y1))
            self._win.draw_Line(line, fill_color)
            line = Line(Point(to_x_mid, to_cell._y2), Point(to_x_mid, to_y_mid))
            self._win.draw_Line(line, fill_color)
        
        # moving down
        elif(self._y1 < to_cell._y1):
            line = Line(Point(x_mid, y_mid), Point(x_mid, self._y2))
            self._win.draw_Line(line, fill_color)
            line = Line(Point(to_x_mid, to_y_mid), Point(to_x_mid, to_cell._y1))
            self._win.draw_Line(line, fill_color)
    
    

